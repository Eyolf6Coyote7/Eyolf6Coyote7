...
export class BoardController {
  ...
  create(@CurrentUser() user: AuthUser, @Body() dto: CreateBoardDto) {
    return this.boardService.create(user.userId, user.tenantId, dto.title, dto.templateId);
  }
  ...
  share(@CurrentUser() user: AuthUser, @Param('id') id: string, @Body() dto: ShareBoardDto) {
    return this.boardService.generateShareLink(id, user.tenantId, dto.permission);
  }

  @Delete(':id/share')
  revokeShare(@CurrentUser() user: AuthUser, @Param('id') id: string) {
    return this.boardService.revokeShareLink(id, user.tenantId);
  }
}
...
export class GuestController {
  ...
  findByGuestToken(@Param('token') token: string) {
    return this.boardService.findByGuestToken(token);
  }
}
